...
def assess_flexibility(task, approach, demand, p_ranges_prods: list = None, p_step_productions: list = None,
    ...
    max_demand = sum(p for p in p_max_productions if p > 0) + sum(p_max_discharges) + sum(p_max_imposed) \
                 + max(min(flexi_dsm), -min(flexi_dsm))
    ...
    print("Power demand = {}".format(p_demand))
    ...
    toc_overall = time.time()  # Initialize clock
...
def build_demand_dictionary(max_range, min_range, step):
    ...
    demand_file = open(work_path + "/demand_file_illustrative_case.txt", "r")
...
# --------------------------------------------------------
# THE SECTION BELOW ALLOWS THE DECLARATION OF A STUDY CASE
# --------------------------------------------------------
...
# -------------------------------------
# MULTIPLE UNITS SCENARIO (FOR TESTING)
# -------------------------------------

# tasks = ["Distribution"]
# approaches = ["Operational"]
# nb = 4
# piloted_productions_unit_names = ["P-" + str(n) for n in range(1, nb + 1)]
# piloted_productions_power_ranges = [[1, 2]] * nb  # [[P_min_1, P_max_1], [P_min_2, P_max_2], ... , [P_min_n, P_max_n]
# piloted_productions_power_steps = [1] * nb  # Must be in decimal, e.g.: 0.1 to express 10% from total p_range
# gcd_step = 1
# dissipation_unit_names = ["D-01"]
# dissipation_power_ranges = [[-1]]
# dissipation_power_steps = [1]
# storage_units_names = ["S-01"]
# storage_units_power_ranges = [[-1, 1]]  # [[P_max_charge1, P_max_discharge1], ..., [P_max_chargeN, P_max_dischargeN]]
# storage_units_power_steps = [[1]]
# storage_discharge_unit_names = ["Discharge-1"]
...
# ---------------------------------------------------
# FRENCH NUCLEAR PLANTS SCENARIO (FOR THE INTERNSHIP)
# ---------------------------------------------------
