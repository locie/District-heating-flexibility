...
def Create_list_easy(df, nb_elements):
    unit_list=[]
    for i in range(len(df)):
        ...
        if  df.loc[i,"production_type"][0:5] == "HYDRO" and  df.loc[i,"unit_type"] == "PRODUCTION_UNIT":
            ...
            unit_list = np.linspace(-installed_p, 0, nb_elements)
            unit_list = np.round(liste).astype(int).tolist()
            unit_list = list(set(liste))
        else:
            installed_p = df.loc[i,"installed_capacity"]
            unit_list = np.linspace(0, installed_p, nb_elements)
            unit_list = np.round(liste).astype(int).tolist()
            unit_list = list(set(liste))
    print(unit_list, type(unit_list))
    return (unit_list)

#list_unit= [list(range(0,500))]*400

"""
unit1=[[0,1,2,3,4]]

unit2=[[0,1,2,3,4],[-1,0,6]]

"""



f1=build_Op_flex(list_unit)
"""
f2=build_Op_flex(unit2)

f3 = Sub_Unit(f2, unit1[0])
